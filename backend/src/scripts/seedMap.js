...
export async function seedMap() {
  // Spalten-Definition: Spalte → [Feld-Positionen]
  const columns = {
    0:  [0, 1, 2, 3],
    1:  [4, 5, 6, 7, 8],
    2:  [9, 10, 11, 12, 13],
  ...
  // x-Koordinaten aus Spalten-Definition aufbauen
  const xCoords = {};
  for (const [x, positions] of Object.entries(columns)) {
    positions.forEach((pos) => {
      xCoords[pos] = parseInt(x);
    });
  }

  // y-Koordinaten aus koordinaten.txt
  const yCoords = {
    0: 0, 1: 3, 2: 4, 3: 6,
    4: 0, 5: 1, 6: 3, 7: 4, 8: 6,
    9: 0, 10: 1, 11: 2, 12: 5, 13: 6,
    14: 1, 15: 2, 16: 4, 17: 6,
    18: 1, 19: 5,
    20: 0, 21: 1, 22: 4, 23: 6,
    24: 0, 25: 1, 26: 2, 27: 3, 28: 6,
    29: 0, 30: 1, 31: 2, 32: 4, 33: 5,
    34: 0, 35: 1, 36: 3, 37: 4,
    38: 0, 39: 2, 40: 4,
    41: 0, 42: 1, 43: 2, 44: 3, 45: 4, 46: 5,
    47: 0, 48: 2, 49: 3, 50: 5,
    51: 1, 52: 2, 53: 5,
    54: 1, 55: 2, 56: 3, 57: 4,
    58: 1, 59: 3,
  ...
  const coords = {};
  for (let i = 0; i <= 60; i++) {
    coords[i] = { x: xCoords[i], y: yCoords[i] };
  }
  ...
  console.log('Map "Dungeon of Knowledge" mit korrekten Koordinaten eingefügt!');
}

// Direktausführung via `npm run seed:map` weiterhin unterstützen
if (import.meta.url === `file://${process.argv[1]}`) {
  const { connectMongo } = await import('../config/db.mongo.js');
  await connectMongo();
  await seedMap();
  process.exit(0);
}