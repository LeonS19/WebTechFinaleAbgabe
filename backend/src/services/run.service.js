...
export async function startRun(userId, studyGroupId, selectedStartFieldPosition) {
  ...
  const availableCards = await IndexCardService.getIndexCards(studyGroupId, undefined, undefined, undefined, userId);
  if(availableCards.length < MIN_CARD_AMOUNT){
    throw new Error(`Die Lerngruppe braucht mindestens ${MIN_CARDS_REQUIRED} Karteikarten, um einen Run zu starten`)
  }
  ...
  const field = map.fields.find(f => f.position === selectedStartFieldPosition);
  if(!field){
  ...
  const run =  await createRun(userId, studyGroupId, map.id, selectedStartFieldPosition);
...
export async function moveToField(runId, userId, targetPosition) {
  ...
  const map = await getMap();
  const currentField = map.fields.find(f => f.position === run.currentPosition);
...
export async function endRun(runId, userId, successful) {
  ...
  const durationInSeconds = Math.round((Date.now() - new Date(run.startTime).getTime()) / 1000);

  return await endRunModel(runId, successful, durationInSeconds);
}