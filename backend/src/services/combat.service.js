...
import { drawCards, discardCard, reshuffleDiscardIntoDeck, findRunDeck, returnCardsToDeck } from './runDeck.service.js';
import { findRunById, updateHealthAndAnswers, applyLevelUp, endRun as endRunModel } from '../models/sql/run.model.js';
import * as IndexCardService from './indexCard.service.js';
import { takeDamage, heal, levelUp, calculateDamageMultiplier } from './utils/playerStats.util.js';
...
function calculateCardDamage(card, studyGroupId) {
  const groupStat = card.group_stats.find(s => s.study_group_id === studyGroupId);
...
export async function answerCard(runId, userId, cardId, userAnswer) {
    ...
    const cardInHand = combat.hand.some(id => id.toString() === cardId.toString());
    if (!cardInHand) {
    ...
    const isCorrect = card.answer.toLowerCase().trim() === userAnswer.toLowerCase().trim();
    ...
    await IndexCardService.recordAnswer(cardId, userId, run.studyGroupId, isCorrect);
    ...
    const wasPerfectRound = combat.turn_start_hand_size === 5 && combat.hand.length === 0 && isCorrect;
    ...
    if (combat.enemy.current_health <= 0) {
        ...
        levelUp(updatedRun);
        await applyLevelUp(runId, { level: updatedRun.level, maxHealth: updatedRun.maxHealth, currentHealth: updatedRun.currentHealth });
        await updateHealthAndAnswers(runId, { currentHealth: updatedRun.currentHealth, correctAnswers, totalAnswers });
        ...
        return { correct: isCorrect, damageDealt, correctAnswer: card.answer };
    }
    ...
    if (turnEnds) {
        ...
        if (updatedRun.currentHealth <= 0) {
            ...
            await updateHealthAndAnswers(runId, { currentHealth: 0, correctAnswers, totalAnswers });
        ...
        const updatedHand = wasPerfectRound ? newCards : [...combat.hand, ...newCards];
